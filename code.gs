...
function getAbsensi(params) {
  ...
  function normWaktu(val) {
    if (!val && val !== 0) return "-";
    if (val instanceof Date) return pad(val.getHours()) + ":" + pad(val.getMinutes()) + ":" + pad(val.getSeconds());
    const s = val.toString().trim();
    return s === "" ? "-" : s;
  }
  for (let i = 1; i < data.length; i++) {
    if (!data[i][0]) continue;
    const tgl = normTgl(data[i][4]);
    const row = {
      id:      data[i][0].toString(),
      ...
      tanggal: tgl,
      waktu:   normWaktu(data[i][5]),
      status:  data[i][6].toString(),
      lat:     data[i][7],
      lng:     data[i][8],
      jarak:   data[i][9],
      catatan: data[i][10] ? data[i][10].toString() : ""
    };
...
function getRekapBulan(params) {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (!rekap[nis]) rekap[nis] = { nis, nama: data[i][2], kelas: data[i][3], hadir: 0, terlambat: 0, izin: 0, sakit: 0, alpha: 0 };
    if (data[i][6] === "Hadir")      rekap[nis].hadir++;
    else if (data[i][6] === "Terlambat") rekap[nis].terlambat++;
    else if (data[i][6] === "Izin")       rekap[nis].izin++;
    else if (data[i][6] === "Sakit")      rekap[nis].sakit++;
    else if (data[i][6] === "Alpha")      rekap[nis].alpha++;
  }
...
// ==================== ABSEN MANUAL (IZIN / SAKIT / ALPHA) ===
function absenManual(body) {
  // body: { nis, nama, kelas, tanggal, status, catatan }
  // status yang valid: "Izin", "Sakit", "Alpha"
  const validStatus = ["Izin", "Sakit", "Alpha"];
  if (!validStatus.includes(body.status)) {
    return { status: "error", message: "Status tidak valid. Gunakan: Izin, Sakit, atau Alpha" };
  }
  if (!body.nis || !body.nama || !body.tanggal) {
    return { status: "error", message: "Data tidak lengkap (nis, nama, tanggal wajib diisi)" };
  }

  const sh = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(CONFIG.SHEET_ABSEN);
  if (!sh) return { status: "error", message: "Sheet Absensi tidak ditemukan. Jalankan initSheets() dulu." };

  // Cek duplikat — siswa sudah punya record di tanggal yang sama
  // Tanggal di sheet bisa berupa Date object atau string, normalisasi ke "YYYY-MM-DD"
  const rows = sh.getDataRange().getValues();
  const pad  = n => String(n).padStart(2, "0");
  function normTgl(val) {
    if (!val) return "";
    if (val instanceof Date) {
      // Gunakan UTC agar tidak kena timezone shift
      return val.getFullYear() + "-" + pad(val.getMonth()+1) + "-" + pad(val.getDate());
    }
    return val.toString().trim().substring(0, 10); // ambil YYYY-MM-DD saja
  }
  for (let i = 1; i < rows.length; i++) {
    if (!rows[i][0]) continue;
    if (rows[i][1].toString() === body.nis.toString() &&
        normTgl(rows[i][4]) === body.tanggal.toString().trim()) {
      return {
        status: "error",
        message: `Siswa ${body.nama} sudah memiliki keterangan absen pada tanggal ${body.tanggal}`
      };
    }
  }
  ...
  sh.appendRow([
    ...
    body.kelas || "",
    body.tanggal,
    "-",           // waktu: tidak ada jam absen manual
    body.status,   // "Izin" / "Sakit" / "Alpha"
    0,             // latitude
    0,             // longitude
    0,             // jarak
    body.catatan || ""
  ...
  return {
    status: "ok",
    message: `Keterangan ${body.status} untuk ${body.nama} pada ${body.tanggal} berhasil disimpan`
  };
}
